...
1. To start a new GitHub repo from this template, at the top of this repo's root page select the "Use this template" option.

[![](assets\img\index_useTemplate.png)

1. Specify a name for your new project, and then create the repo. On the new repo's page, copy the cloning address from the "Code" dropdown menu (indicated below as `<repo-url>`. Use HTTPS or SSH depending on your setup for Git. If you have never used Git and need further instructions, see the [Configuring Git]() tutorial before continuing.

1. On your machine, open a Bash (or other) terminal in a directory of your preference and execute the command `git clone <repo-url>` The remainder of these basic usage instructions assume that all suggested actions are taken from this new repository.
