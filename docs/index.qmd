...
:::{.callout-important}
Docs builds are now fully automated. The separate `write_docs` directory is no longer needed. Simply create/edit docs files in the `docs` folder (leave content in `docs/build-docs` alone, this folder contains the rendered site) and commit/push changes as normal. The build/deploy workflow will run automatically. For more info, see [Documentation](tools/documentation.qmd#authoredit-documentation)
:::
...
For documentation details, including how to edit documentation and generate a website, see the [Documentation tools](tools/documentation.html) page.
