...
Ready to give it a try? See our [primer on installing and setting up VS Code](tools/vscode_primer.qmd) to effectively collaborate on Python development projects. 
...
### Venv
[venv](https://docs.python.org/3/library/venv.html) is a core module in Python, which enables creating "virtual environments" that each have their own set of Python packages installed in their site directories. A venv is created on top of an existing Python installation (known as the 'base' Python). A venv may be created from a `requirements.txt` file that specifies the required packages and their versions.

## Documentation
Documentation is a vital part of any Python project. See the [documentation tools page](tools/documentation.qmd) for a review of using some recommended tools for documenting your projects.
